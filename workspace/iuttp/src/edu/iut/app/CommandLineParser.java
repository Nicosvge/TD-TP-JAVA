			...
			if (options.containsKey(keyValue[0])) {
				switch (((CommandLineOption<?>)options.get(keyValue[0])).getOptionType()) {
					case FILE:
						...
						break;
					case STRING:
						CommandLineOption<String> fileOption2 = (CommandLineOption<String>)options.get(keyValue[0]);
						if (keyValue.length == 2) {
							fileOption2.setValue(new String(keyValue[1]));
						}
						else {
							parseErrors.add("Option should have a key and a value.");
						}
						break;
					case INTEGER:
						CommandLineOption<Integer> fileOption3 = (CommandLineOption<Integer>)options.get(keyValue[0]);
						if (keyValue.length == 2) {
							fileOption3.setValue(new Integer(keyValue[1]));
						}
						else {
							parseErrors.add("Option should have a key and a value.");
						}
